"""NL4OPT HuggingFace dataset benchmark tests — require a live LLM API key."""
...
@pytest.mark.llm
@pytest.mark.NL4OPT
def test_NL4OPT_sample(llm_fixture, save_dir, generate_ir, start, limit, solver):
    """Load NL4OPT cases and run the direct code gen pipeline for each.
    ...
    """
    pytest.importorskip("datasets", reason="pip install 'orpilot[hf]' to run NL4OPT tests")

    from orpilot.benchmark.loader_hf import load_nl4opt_cases

    cases = load_nl4opt_cases(offset=start, limit=limit)
    assert cases, "No cases loaded from CardinalOperations/NL4OPT"
    ...
    log.info("Running %d NL4OPT cases (mode=%s, limit=%s)", len(cases), mode_label, limit)
    ...
    assert not failures, f"{len(failures)}/{total} NL4OPT cases failed:\n" + "\n".join(failures)
