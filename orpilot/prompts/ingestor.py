...
SYSTEM_PROMPT = """\
You are an expert Operations Research analyst. Your task is to parse a plain-text OR problem \
description that embeds both the problem specification and all data values, and return a single \
JSON object with two top-level keys: "problem" and "tables".
...
4. Scalar parameters (a single number like "capacity = 50") must be stored as a \
single-row table, e.g. `{"capacity": [{"capacity": 50}]}`.
5. Every table must have at least one ID column (the first column is always an ID column \
unless the table is a pure scalar parameter).
